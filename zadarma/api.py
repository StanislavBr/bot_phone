...
from function import get_account_datas
...
class ZadarmaAPI(object):
    ...
    def __get_auth_string_for_header(self, method, params_string):
        ...
        hmac_h = hmac.new(get_account_datas()[1].encode('utf8'), data.encode('utf8'), sha1)
        if sys.version_info.major > 2:
            bts = bytes(hmac_h.hexdigest(), 'utf8')
        else:
            bts = bytes(hmac_h.hexdigest()).encode('utf8')
        auth = get_account_datas()[0] + ':' + base64.b64encode(bts).decode()
        return auth
